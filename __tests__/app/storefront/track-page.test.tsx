...
describe("TrackPage", () => {
  it("renders the heading, tracking-number input and submit button", () => {
    render(<TrackPage />);
    expect(
      screen.getByRole("heading", { level: 1, name: copy.track.title })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("textbox", { name: /tracking number/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: copy.track.trackOnJt })
    ).toBeInTheDocument();
  });

  it("does not redirect when the input is empty and shows an inline hint", async () => {
    render(<TrackPage />);
    await userEvent.click(screen.getByRole("button", { name: copy.track.trackOnJt }));
